...
class npy_data_loader(file_data_loader):
    ...
    def __init__(self, data_dir, prefix, mode, word_vec_npy='vec.npy', shuffle=True, max_length=70, batch_size=50):
...
class json_file_data_loader(file_data_loader):
    ...
    def _load_preprocessed_file(self, processed_data_dir):
    ...
    def __init__(self, processed_data_dir, file_name, word_vec_file_name, rel2id_file_name, mode, shuffle=True, max_length=60,
                 case_sensitive=False, reprocess=False, batch_size=50, word_embedding_dim=50):
        '''
        ...
        if reprocess or not self._load_preprocessed_file(processed_data_dir): # Try to load pre-processed files:
            # Check files
            ...
            print("Sort data...")
            self.ori_data.sort(key=lambda a: a['head']['word'] + '#' + a['tail']['word'] + '#' + a['relation'])
            ...
            for i in range(self.instance_tot):
                ...
                head = ins['head']['word']
                tail = ins['tail']['word']
                cur_entpair = ins['head']['word'] + '#' + ins['tail']['word']
                cur_relfact = ins['head']['word'] + '#' + ins['tail']['word'] + '#' + ins['relation']
    ...
    def next_batch(self, batch_size):
        ...
        if self.mode == self.MODE_INSTANCE:
            ...
            tmp_v = np.zeros((self.rel_tot), dtype=np.int32)
            batch_data['multi_rel'] = [[1 if i==self.data_rel[j] else 0 for i in range(self.rel_tot)] for j in range(idx0, idx1)]
            batch_data['entpair'] = ["Has_Value" for _ in range(idx1-idx0)]
            ...
            for _ in range(batch_size - (idx1 - idx0)):
                batch_data['multi_rel'].append(np.zeros((self.rel_tot), dtype=np.int32))
                batch_data['entpair'].append('None#None')
        elif self.mode == self.MODE_ENTPAIR_BAG or self.mode == self.MODE_RELFACT_BAG:
            idx0 = self.idx
            idx1 = self.idx + batch_size
            tmp_idx = idx1 if self.mode==self.MODE_RELFACT_BAG else self.idx
            if tmp_idx >= len(self.order):
                self.idx = 0
                ...
                raise StopIteration
            self.idx = idx1
            idx1 = len(self.order) if idx1> len(self.order) else idx1
            _word = []
            _pos1 = []
            _pos2 = []
            _mask = []
            ...
            for i in range(idx0, idx1):
                ...
                bag_size = self.scope[self.order[i]][1] - self.scope[self.order[i]][0]
                _scope.append([cur_pos, cur_pos + bag_size])
                _scope_name.append(self.scope_name[self.order[i]].split('#')[0:2])
                cur_pos = cur_pos + bag_size
                if self.mode == self.MODE_ENTPAIR_BAG:
                    _one_multi_rel = np.zeros((self.rel_tot), dtype=np.int32)
                    for j in range(self.scope[self.order[i]][0], self.scope[self.order[i]][1]):
            ...
            first_tuple = 0
            index_count = 0
            if self.mode == self.MODE_RELFACT_BAG:
                while (first_tuple < batch_size):
                    subj1 = _scope_name[first_tuple][0]
                    obj1 = _scope_name[first_tuple][1]
                    second_tuple = 0
                    while (second_tuple < batch_size):
                        if first_tuple >= second_tuple:
                            second_tuple += 1
                            index_count += 1
                            continue
                        subj2 = _scope_name[second_tuple][0]
                        obj2 = _scope_name[second_tuple][1]
                        if subj1 == subj2:
                        ...
                        index_count += 1
                    first_tuple += 1
            batch_data['subj_rel_set'] = np.array(subj_rel_set)
            batch_data['rel_obj_set'] = np.array(rel_obj_set)
            batch_data['subj_rel_obj_set'] = np.array(subj_rel_obj_set)
            batch_data['subj_multi_set'] = np.array(subj_multi_set)
            batch_data['obj_multi_set'] = np.array(obj_multi_set)
