...
class ConstraintLoss:
    def __init__(self, DataLoader, loss_rate, model):
        self.model = model
        self.batch_size = DataLoader.batchsize
        self.class_num = DataLoader.class_num
        self.loss_rate = loss_rate
        self.SemConstraints = self.ConstraintLoader(DataLoader, 'Sem')
        self.CohConstraints = self.ConstraintLoader(DataLoader, 'Coh')
    ...
    def ConstraintLoader(self, DataLoader, CLCMode='Sem'):
        Constraints = {'SubjRel': [],'RelObj': [], 'SubjRelObj': [], 'SubjMulti': [], 'ObjMulti': []}
        rel2id = DataLoader.rel2id
        def merge_type(path):
        ...
        subj_rel_cons_list = merge_type(os.path.join(DataLoader.constraint_path, 'subj_rel.json'))
        rel_obj_cons_list = merge_type(os.path.join(DataLoader.constraint_path, 'rel_obj.json'))
        subj_rel_obj_cons_list = merge_type(os.path.join(DataLoader.constraint_path, 'subj_rel_obj.json'))
        subj_multi_cons_list = merge_card(os.path.join(DataLoader.constraint_path, 'subj_multi.json'))
        obj_multi_cons_list = merge_card(os.path.join(DataLoader.constraint_path, 'obj_multi.json'))
